...
int main(void) {
    ...
    while(1) {
        ...
        if(ROWS == 15 && COLUMNS == 15 ) {
            printf("YOU JUST PROVED YOU ARE A TOP PLAYER!!!\n");
            printf("YOU BEAT EVERY LEVEL ON MINESWEEPER!!!\n");
            sleep(10);
            printf("Closing game");
            sleep(2);
            return 0;
        ...
        do {
            printf("Begin next level?[y or n]\n");
            ans = getchar();
            if(ans != '\n') {
                if(ans == 'n') {
                    printf("Closing the game.\n");
        ...
        }while(ans != 'y' && getchar() != EOF);
        getchar();

        for(i=0;i<ROWS;i++) {
            free(array_visible[i]);
            free(array_main[i]);
        }
        free(array_visible);
        free(array_main);
